...
RSpec.describe ImportNintendoSouthAmericaDlcData, type: :mutations do
  ...
  describe 'Private instance methods' do
    describe '#fetch_data' do
      ...
      before do
        allow(Item).to receive(:from_south_america_with_dlc).and_return([item])
        allow(subject).to receive_message_chain('crawler.crawl_dlc').with(item: item).and_return(dlc_data)
      end

      it 'assigns fetched data to @data_collection' do
        subject.send(:fetch_data)
        expect(subject.instance_variable_get('@data_collection')).to eq [dlc_data]
      end
    end
  end
  ...
  describe '#crawler' do
    ...
    context 'when @crawler is nil' do
      before do
        ...
        allow(NintendoSouthAmericaDlcCrawler).to receive(:new).and_return(crawler)
      end
