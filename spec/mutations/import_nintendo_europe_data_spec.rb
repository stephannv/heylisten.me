...
RSpec.describe ImportNintendoEuropeData, type: :mutations do
  let(:data_type) { %w[game dlc].sample }

  subject { described_class.new(data_type: data_type) }
  ...
  describe 'Private instance methods' do
    describe '#fetch_data' do
      ...
      before do
        allow(FetchNintendoEuropeData).to receive(:run!).with(data_type: data_type).and_return(data_collection)
      end
  ...
  describe '#parse_data' do
    ...
    before do
      ...
      allow(NintendoEuropeDataAdapter).to receive(:new)
        .with(data: data_item, item_type: data_type)
        .and_return(adapter)
    end
