class ImportNintendoBrasilData < Mutations::Command
  def execute
    fetch_data
    parse_data
    import_data
  ...
  private def fetch_data
    @data_collection = FetchNintendoBrasilData.run!
  end

  private def parse_data
    @parsed_data_collection = @data_collection.map do |data|
      NintendoAlgoliaDataAdapter.new(data: data, data_source: :nintendo_brasil).adapt
    end
  end
